...
function initGrid(){
   	...
   	dataGrid.setHeader("&nbsp;,Employer,Alias"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("getEmployerXML.action");
}

function addNewRecord(){
	var url= "addEmployer.action";
	showInPopUp(url,500,300,loadGrid,true);
}

function editRecord(id){
    var url="editEmployer.action?employerId="+id;
	showInPopUp(url,500,300,loadGrid,true);
}
...
function deleteRecord(id){
    ...
    if (retVal == true) {
		var pars = "employerId="+id;
		var myAjax = ajaxCall("deleteEmployer.action","get",pars,onDeleteComplete,reportError);
    }
}
