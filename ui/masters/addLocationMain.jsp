...
<div class="contentDivPop" style="width:450px;">
	...
	<div class="popupTop">
		<table class="tblPop">
		   <tr>
			  <td class="header">
				  <bean:message key="master_locations.label.location_name"/>
				  <span class="star">*</span>
				  :
			  </td>
			  <td>
				  <html:text property="locationName" name="mastersForm" size="35" styleId="skillCategory"></html:text>
			  </td>
		  </tr>
	 </table>
	</div>
...
function submitForm(){
	var Name = document.mastersForm.locationName.value;
	if(Name.trim()==""){
		return;
	}
	document.mastersForm.mode.value='saveLocation';
	document.mastersForm.submit();
}

function actionOnLoad(){
	<logic:empty name="mastersForm" property="locationId">
		window.top.setPopTitle('<b><bean:message key="master_locations.label.add_location"/></b>');
	</logic:empty>
	<logic:notEmpty name="mastersForm" property="locationId">
		window.top.setPopTitle('<b><bean:message key="master_locations.label.edit_location"/></b>');
	</logic:notEmpty>
}
