...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"branch");
			break;	
		case 2:
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;">Branches Master</td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td> 
...
function exportToExcel(){
	var url = "export.do?mode=exportMasters&exportEntityType=<%= ExportConstants.ENTITY_BRANCHES%>&ids="+dataGrid.getAllItemIds();
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}

function importFromExcel(){
	var url = "import.do?mode=importMasters&importEntityType=<%= ExportConstants.ENTITY_BRANCHES%>";
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="admin_master_branch_cat.label"/>,<bean:message key="admin_master_alias_cat.label"/>"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageBranches&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function addNewRecord(){
	var url="masters.do?mode=manageBranchesMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,300,loadGrid,true);
}

function editRecord(id){
    var url="masters.do?mode=manageBranchesMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&branchName=&branchId="+id;
	showInPopUp(url,500,300,loadGrid,true);
}


function deleteRecord(id){
    var val = dataGrid.getUserData(id,"branch");
    retVal = confirm('<bean:message key="admin_master_dept.label.confirm_delete_department"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=manageBranches&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&branchId="+id;
...
function startExcelImport(filePath){	
	...
	document.importForm.entityType.value='<%= ExportConstants.ENTITY_BRANCHES%>';
