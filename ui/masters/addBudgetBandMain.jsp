...
<div class="contentDivPop" style="width:450px;">
	...
	<html:form action="/masters" onsubmit="submitForm();return false;">
  	<html:hidden property="budgetBandId" name="mastersForm"/>
	...
	<div class="popupTop">
		<table class="tblPop">
		   <tr>
			  <td class="header">
				  <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUDGET_ITEM_BAND_LABEL) %> <bean:message key="master_budget_grades.label.name"/>
				  <span class="star">*</span>
				  :
			  </td>
			  <td>
				  <html:text property="budgetBandName" name="mastersForm" size="35" styleId="budgetBandName"></html:text>
			  </td>
		  </tr>
		  <tr>
			  <td class="header">
...
function submitForm(){
	var errors = '';
	var Name = document.mastersForm.budgetBandName.value;
	if(Name.trim()==""){
		//return;
...
function actionOnLoad(){
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_ADD%>">
		window.top.setPopTitle('<b><bean:message key="common.add"/> <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUDGET_ITEM_BAND_LABEL) %></b>');
	</logic:equal>
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_EDIT%>">
		window.top.setPopTitle('<b><bean:message key="common.edit"/> <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUDGET_ITEM_BAND_LABEL) %></b>');
	</logic:equal>
}
