	    ...
	    <td  valign="bottom">
			...
			<tr>
				...
				<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;">
					<bean:message key="admin_master_label_budget"/> <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUDGET_ITEM_BAND_LABEL) %> <bean:message key="admin_master_label_master"/>
				</td>
				<td class="rightC"></td>
			</tr>
			</table>
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUDGET_ITEM_BAND_LABEL) %> <bean:message key="master_budget_grades.label.name"/>,<bean:message key="master_budget_grades.label.grade_description"/>"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageBudgetBands&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function addNewRecord(){
	var url="masters.do?mode=manageBudgetBandMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,180,loadGrid,true);
}

function editRecord(id){
    var url="masters.do?mode=manageBudgetBandMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&budgetBandId="+id;
	showInPopUp(url,500,180,loadGrid,true);
}
...
function deleteRecord(id){
    ...
    if (retVal == true) {
		var pars = "mode=manageBudgetBands&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&budgetBandId="+id;
