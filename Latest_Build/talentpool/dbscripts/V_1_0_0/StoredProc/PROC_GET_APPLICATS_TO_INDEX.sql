...
SET @qry = "CREATE TEMPORARY TABLE tmp_applicants (
	  applicant_id bigint(20) ,                            
          ...
          processed int(11) ,                               
          hDegree varchar(1000),                                                        
          oDegree varchar(1000)
	)
	...
	( select CONCAT(IFNULL(td.degree_title,' '),'||', IFNULL(tb.branch_name,' '), '||',
		IFNULL(ti.institute_name,' '), '||' ,IFNULL(i_taei.educational_info_year_of_passing,' ') )
		from tp_applicant_educational_info i_taei 
		LEFT JOIN tp_degrees td ON(td.degree_id = i_taei.degree_id)
		LEFT JOIN tp_branches tb ON(tb.branch_id = i_taei.branch_id)
		LEFT JOIN tp_institutes ti ON(ti.institute_id = i_taei.institute_id)
		WHERE i_taei.applicant_id=ta.applicant_id and i_taei.educational_level=0
	) as hDegree ,
	...
	( select CONCAT(IFNULL(td.degree_title,' '),'||', IFNULL(tb.branch_name,' '), '||',
		...
		LEFT JOIN tp_branches tb ON(tb.branch_id = i_taei.branch_id)
		LEFT JOIN tp_institutes ti ON(ti.institute_id = i_taei.institute_id)
		WHERE i_taei.applicant_id=ta.applicant_id and i_taei.educational_level=1
	) as oDegree
	";
