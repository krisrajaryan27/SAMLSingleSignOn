...
/**
 * custom method to delete row from backup  #Ajeet#
 * */
function removeIdFromBackUp(dataGrid, text){
	if(dataGrid._f_rowsBuffer){
		for(var i=dataGrid._f_rowsBuffer.length-1; i>=0; i--){
			if(dataGrid._get_cell_value(dataGrid._f_rowsBuffer[i],0).toString()==text)
				dataGrid._f_rowsBuffer.splice(i,1);
		}
	}
}
/**
 * custom method to reset filter backup #Ajeet#
 * */
function resetFilterBackUp(dataGrid){
	dataGrid._f_rowsBuffer=null;
}
