...
dhtmlXGridObject.prototype._pgn_block=function(sep){ 
	...
	for (var i=start+1; i<=max; i++)
		str.push("<a onclick='this.grid.changePage("+i+"); return false;'>"+(i==this.currentPage?("<b>"+i+"</b>"):i)+"<a>");
	return str.join(sep);
}
dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
	if (mode=="prevpages" || mode=="prev"){
		if (this.currentPage==1) return ds;
		return '<a onclick=\'this.grid.changePageRelative(-1*this.grid.pagesInGroup); return false;\'>'+ac+'</a>'
	}
	...
	if (mode=="nextpages" || mode=="next"){
		...
		return '<a onclick=\'this.grid.changePageRelative(this.grid.pagesInGroup); return false;\'>'+ac+'</a>'
	}
