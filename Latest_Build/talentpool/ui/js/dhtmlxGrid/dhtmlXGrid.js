...
function dhtmlXGridObject(id){
	...
	this.objBox=document.createElement("DIV");
	 /* modifications by shiv starts*/
	 //this.objBox.style.width="100%";
	 var wdth=this.entBox.style.width;
	 if(wdth!=""){
	 	wdth = wdth.substring(0,wdth.length-2)-2 + "px";
	 }
	 this.objBox.style.width =wdth;
	 /* modifications by shiv ends*/
	...
	*/
	this.clearAll=function(header){
	   ...
	   this._hrrar=new Array();*/
		...
		//modified by shiv, sorting state was not maintained after loadXML
		//this.setSortImgState(false);
		//modified by shiv -End
		this.setSizes();
		//this.obj.scrollTop = 0;
...
//#}	
	...
	*/			
	load:function(url, call, type){
		/**
		 * AJEET START
		 * code modify to append timestamp in each request
		 */
...
//#}
		...
		/* added by shiv */
			 ...
			 s[sort](function(a, b){
				 if(order=="asc")
				 	return arrTS[a.idd].toLowerCase()>arrTS[b.idd].toLowerCase()?1:-1
				 else
				 	return arrTS[a.idd].toLowerCase()<arrTS[b.idd].toLowerCase()?1:-1
			 });
		}
...
/** 
*     METHOD ADDED BY SHIV
*/
dhtmlXGridObject.prototype.checkSessionExpiry = function(flag, urlToRedirect){
...
*     METHOD ADDED BY SHIV
*/
dhtmlXGridObject.prototype.setHeaderCursor = function(curStr){
 var arLab = curStr.split(this.delim);
 for(var i=0;i<arLab.length;i++){
      if(arLab[i]!=""){
            this.hdr.rows[1].cells[i].firstChild.style.cursor = arLab[i];
      }
 }
}