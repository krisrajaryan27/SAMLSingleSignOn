...
<body>
<s:form name="positionStatusScheduler" method="POST" action="savePositionStatusProperties" >
			 		...
			 		<script>
						var opts = [new SelectOption('4','Every 4 hrs'),
						            ...
						            new SelectOption('24','Every 24 hrs'),
										new SelectOption('48','Every 2 day'),
							            new SelectOption('72','Every 3 days'),
							            new SelectOption('96','Every 4 days'), 
							            new SelectOption('120','Every 5 days'), 
							            new SelectOption('168','Every 7 days')];
...
function save(){
	document.positionStatusScheduler.startHour.value = selectBoxStartHour.getSelectedId() + ":00";
	document.positionStatusScheduler.frequency.value = selectBoxFrequency.getSelectedId();
	document.positionStatusScheduler.submit();
}

function runNow(){
	showUpdater('wait',{setHeight: false, setWidth: false, offsetLeft: 0});
	$('curStat').replace(': &nbsp;&nbsp;RUNNING');
	document.positionStatusScheduler.action='runPositionStatusScheduler.action';
	document.positionStatusScheduler.submit();
}
