		...
		<tr>
			<td>
				<div class="navBtnTab" style="width:170px;float: left;">
						...
						<a href="#" onclick="javascript: publishToWebsite();" style="width:140px;"><span class="rightC"></span><span class="leftC"></span><bean:message key="common.publish"/> /  <bean:message key="position.publish.label.unpublish"/> </a>
				</div>
			</td>
			<td>
...
function initGrid(){	
	...
	publishPositionsGrid.imgURL = "images/"; 
   	publishPositionsGrid.setHeader(",<bean:message key="common.position_code" />,<bean:message key="common.position_title" />,<bean:message key="position.publish_to_web_site.label.position_location" />,<bean:message key="position.publish_to_web_site.label.position_rule" />,<bean:message key="position.publish_to_web_site.label.position_status" />");
   	publishPositionsGrid.attachHeader("&nbsp;,#text_filter,#text_filter,#text_filter,#text_filter,#select_filter_strict"); 
   	publishPositionsGrid.setInitWidths("10,189,189,120,120,89");
...
function loadGrid(){
	...
	publishPositionsGrid.loadXML("position.do?mode=XMLforPublishPositionsToWebsite");
}
...
function publishToWebsite(){
	var positionId = publishPositionsGrid.getSelectedId();
   	if(positionId){	
		var url="position.do?mode=publishPositionToSite&positionId="+positionId;
...
function reloadWindow(){
	window.location="position.do?mode=getAllPositionsToPublishToWebsite";
}
