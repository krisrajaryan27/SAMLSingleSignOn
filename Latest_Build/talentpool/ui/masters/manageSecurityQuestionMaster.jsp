...
<div class="contentDiv">
	<table style="width: 100%; border: 0; border-spacing: 0; padding: 0"> 
	  <tr> 
	    <td valign="bottom">
			<table style="border-spacing: 0; border: 0;" cellpadding="0" class="boxETab">
			<tr>
				<td class="leftC"></td>
				<td class="content Grey" style="padding-left:10px; padding-right:10px;">
					<bean:message key="admin_master.label.security_question_master"/>
				</td>
				<td class="rightC"></td>
			</tr>
			</table>
		...
		<div class="navBtnTab" style="width:105px;float: right;">
			...
			<a href="#" style="width:80px;" onclick="addNewRecord();">
				<span class="rightC"></span><span class="leftC"></span><bean:message key="common.add_new"/>
			</a> 
		</div>			    
	    </td> 
	  </tr> 
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="admin_master.title.security_question"/>"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("getSecurityQuestionXML.action");
}

function addNewRecord(){
	var url= "addSecurityQuestion.action";
	showInPopUp(url,500,180,loadGrid,true);
}

function editRecord(id){
    var url="editSecurityQuestion.action?securityQuestionId="+id;
	showInPopUp(url,500,180,loadGrid,true);
}
...
function deleteRecord(id){
    ...
    if (retVal == true) {
		var pars = "securityQuestionId="+id;
		var myAjax = ajaxCall("deleteSecurityQuestion.action","get",pars,onDeleteComplete,reportError);
    }
}
