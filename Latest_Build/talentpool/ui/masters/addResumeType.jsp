...
<s:form action="saveResumeType" method="POST">
<s:hidden name="resumeTypeId" id="resumeTypeId"/>
	<div class="popupTop">
		<table class="tblPop">
		   <tr>
			  <td class="header"><s:label key="title.common"/><span class="star">*</span>:
			  </td>
			  <td>
			  	  <s:textfield id="resumeType" name="resumeType" size="65" maxLength="250"/>
			  </td>
		  </tr>
	 </table>
	</div>
...
function submitForm(){
	var Name = $('resumeType').value;
	if(Name.trim()==""){
		alert('Please enter Resume Type');
		return;
	}
	document.saveResumeType.submit();
}

function actionOnLoad(){	
		window.top.setPopTitle('<b>Resume Type</b>');
}
