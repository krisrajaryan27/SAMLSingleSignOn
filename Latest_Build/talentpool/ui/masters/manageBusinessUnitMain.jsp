...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"buName");
			break;
	}
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_BUSINESS_UNIT_LABEL)%>,<bean:message key="master_business_unit.label.description"/>"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageBusinessUnit&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function addNewRecord(){
	var url="masters.do?mode=manageBusinessUnitMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,180,loadGrid,true);
}

function editRecord(id){
    var url="masters.do?mode=manageBusinessUnitMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&buId="+id;
	showInPopUp(url,500,180,loadGrid,true);
}


function deleteRecord(id){
    var val = dataGrid.getUserData(id,"buName");
    retVal = confirm('<bean:message key="admin_master_dept.label.confirm_delete_department"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=manageBusinessUnit&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&buId="+id;
