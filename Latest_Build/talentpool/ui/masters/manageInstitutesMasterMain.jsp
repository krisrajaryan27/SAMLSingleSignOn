...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"institute");
			break;	
		case 2:
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;">Institutes Master</td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td> 
...
function exportToExcel(){
	var url = "export.do?mode=exportMasters&exportEntityType=<%= ExportConstants.ENTITY_INSTITUTES%>&ids="+dataGrid.getAllItemIds();
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="admin_master_institute_cat.label"/>,<bean:message key="admin_master_alias_cat.label"/>"); 
	   	dataGrid.setInitWidths("18,240,460");
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageInstitutes&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function addNewRecord(){
	var url="masters.do?mode=manageInstitutesMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,300,loadGrid,true);
}

function editRecord(id){
    var url="masters.do?mode=manageInstitutesMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&instituteName=&instituteId="+id;
	showInPopUp(url,500,300,loadGrid,true);
}

function deleteRecord(id){
    var val = dataGrid.getUserData(id,"institute");
    retVal = confirm('<bean:message key="admin_master_inst.label.confirm_delete_institute"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=manageInstitutes&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&instituteId="+id;
...
function onDeleteComplete(request){
	...
	if(isErrorXml(xmlFile)){
		alert('<bean:message key="admin_master_institute.error.failed_delete"/>');
		return;
	}
...
function importFromExcel(){
	var url = "import.do?mode=importMasters&importEntityType=<%= ExportConstants.ENTITY_INSTITUTES%>";
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}
...
function startExcelImport(filePath){	
	...
	document.importForm.entityType.value='<%= ExportConstants.ENTITY_INSTITUTES%>';
