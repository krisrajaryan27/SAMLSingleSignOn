<%@ taglib uri="/WEB-INF/tld/struts-html.tld" prefix="html"%>
<%@ taglib uri="/WEB-INF/tld/struts-bean.tld" prefix="bean"%>
<%@ taglib uri="/WEB-INF/tld/struts-logic.tld" prefix="logic"%>
<%@page import="java.util.ArrayList"%>
<%@page import="com.talentPool.masters.dataobject.MultipleSelectFieldsData"%>
<%@page import="org.apache.struts.Globals"%>

<script language="JavaScript" src="js/scripta/lib/prototype.js" type="text/javascript"></script>	
<script language="JavaScript" src="js/commonFunctions.js" type="text/javascript"></script>													
<script language="JavaScript" src="js/doClasses/RatingField.js" type="text/javascript"></script>
<script language="JavaScript" src="js/doClasses/RatingData.js" type="text/javascript"></script>
											
<script language="JavaScript">
var multipleSelectFields = new Array();
<% 
ArrayList multipleSelectFields = (ArrayList)request.getAttribute("multipleSelectFields");
for(int i=0; multipleSelectFields!=null && i<multipleSelectFields.size(); i++){
	//create javascript array
	MultipleSelectFieldsData multipleSelectFieldsData = (MultipleSelectFieldsData)multipleSelectFields.get(i);
	String fieldId = multipleSelectFieldsData.getSelectFieldId();
	String fieldDesc = multipleSelectFieldsData.getSelectFieldDesc().replaceAll("'","\\\\'");
%>	
var objR = new RatingField('<%=fieldId%>','<%=fieldDesc%>');
multipleSelectFields[multipleSelectFields.length]=objR;
<%	
}
%>

</script>
<div class="contentDiv">
	<div id="divError" style="display:block">
	<% 
		if(request.getAttribute(Globals.ERROR_KEY)!=null){
	%>
	<table  id="m_errortable" > 
		<tr>
		  <td class="header">
		    <b><bean:message key="errors.following_errors"/></b>
		  </td>               
		</tr>
		<tr>
		    <td class="message"><html:errors/></td>               
		</tr>
	</table>
	<br>
	<% } %>
	</div>
<html:form action="/masters">
	<html:hidden property="mode" name="mastersForm"/>
	<html:hidden property="subMode" name="mastersForm"/>
	<html:hidden property="multipleSelectId" name="mastersForm"/>
	<html:hidden property="multipleSelectFieldsString" name="mastersForm"/>
	<html:hidden property="submitted" name="mastersForm" value="1"/>
	
	<table border="0" cellspacing="0" cellpadding="0" class="posinput">
		<tr>
			<td class="label"><bean:message key="master_add_multiple_select.label.select_name" />
			<span class="star">*</span></td>
			<td><html:text name="mastersForm" property="multipleSelectTitle" size="50"/></td>
		</tr>
	</table>
	<br/>
	
	<div style="cursor: pointer;" class="boxDarkTab" onclick="javascript: toggleFolderDisplay(2)"><span class="rightC"></span><span class="leftC"></span>&nbsp;<bean:message key="master_add_multiple_select.label.multiple_select_fields"/></div>
	<div class="outerDiv" style="padding:0px 0px 5px 0px">
	<table id="fields" cellspacing="0" cellpadding="0" class="posinput">
	<tbody>
	<tr>
		<td style="width:20px;" class="theader">&nbsp;</td>
		<td class="label theader" style="width:40px;"><bean:message key="master_add_multiple_select.label.multiple_select_rank" /></td>
		<td class="label theader" style="width:300px;"><bean:message key="master_add_multiple_select.label.multiple_select_desc" /></td>
		<td style="width:30px;" class="theader">&nbsp;</td>
		<td style="width:30px;" class="theader">&nbsp;</td>
	</tr>	
	</tbody>
	</table>
	<br/>
	<table border="0" cellspacing="0" cellpadding="0" class="posinput">
		<tr>
			<td>
				<a href="#" style="width:220px;" class="btn3" onclick="javascript:addNewField();"><span class="rightC"></span><span class="leftC"></span><bean:message key="master_add_multiple_select.label.add_new_option"/></a>
			</td>
		</tr>
	</table>
	</div>
	<br/>
	<br/>
	<div class="navBtn" style="float:right;">
		<a href="#" style="width:50px;" class="active" onclick="javascript:saveMultipleSelect();"><span class="rightC"></span><span class="leftC"></span><bean:message key="common.save"/></a>
		<a href="#" style="width:60px;margin-left:5px;" class="active" onclick="javascript:cancelOperation();"><span class="rightC"></span><span class="leftC"></span><bean:message key="common.cancel"/></a>
	</div>
	
</html:form>
</div>
<script language="JavaScript">
function populateFields(){
	tbl = $('fields');
	if (tbl) {
		var tBody= tbl.getElementsByTagName("tbody")[0];
		if(tBody){
			var tRows = tBody.childNodes;
			for(var i = tRows.length - 1; i >= 1; i--){
				tBody.removeChild(tRows[i]);
			}	
		}
		if(multipleSelectFields.length>0){
			for (i = 0; i < multipleSelectFields.length; i++) {	
				if(tBody){
				}else{
					tBody = document.createElement("TBODY");
				}
				var tRow = document.createElement("TR");
				
				var tCell0 = document.createElement("TD");
				tCell0.innerHTML = '<img style="cursor:pointer;" title="' + '<bean:message key="common.delete"/>' + '" src="images/ico_delete.gif" onclick="javascript: doDeleteTrait(' + i + ');"/>';		
				tRow.appendChild(tCell0);
				
				tCell0 = document.createElement("TD");
				tCell0.innerHTML = i+1;		
				tRow.appendChild(tCell0);
				
				tCell0 = document.createElement("TD");
				//var el = createFormElement("input","text",'fld_'+ i,multipleSelectFields[i].getFieldDescription(),'');
				//el.maxLength = '255';
				//el.size='50';
				//el.name='fld_'+ i;
				tCell0.innerHTML = '<input type=\'text\' size=\'50\' maxlength=\'255\' id=\'fld_'+i+'\' name=\'fld_'+i+'\' value=\'\' />';
				//tCell0.appendChild(el);
				tRow.appendChild(tCell0);
				
				
				tCell0 = document.createElement("TD");
				tCell0.innerHTML = '<img style="cursor:pointer;" title="' + '<bean:message key="common.move_up"/>' + '" src="images/btn_uparrow.gif" onclick="javascript: swapFields(' + (i - 1) + ', ' + i + ');"/>';		
				tRow.appendChild(tCell0);
				
				tCell0 = document.createElement("TD");
				tCell0.innerHTML = '<img style="cursor:pointer;" title="' + '<bean:message key="common.move_down"/>' + '" src="images/btn_dwnarrow.gif" onclick="javascript: swapFields(' + i + ', ' + (i + 1) + ');"/>';		
				tRow.appendChild(tCell0);

				tBody.appendChild(tRow);
				tbl.appendChild(tBody);
				$('fld_'+i).value=multipleSelectFields[i].getFieldDescription();
			}
		}
		
	}	
	
}
function doDeleteTrait(index){
	updateFieldsArray();
	multipleSelectFields.splice(index, 1);
	populateFields();
}
function swapFields(id1, id2) { 	
	updateFieldsArray();
	if (id2 != 0 && id2 != multipleSelectFields.length) {              
		_temp = multipleSelectFields[id1];
		multipleSelectFields[id1] = multipleSelectFields[id2];
		multipleSelectFields[id2] = _temp;
		populateFields();
	}
}
function addNewField(){
	var objR = new RatingField('0','');
	multipleSelectFields[multipleSelectFields.length]=objR;
	updateFieldsArray();
	populateFields();
}
function updateFieldsArray(){
	for (i = 0; i < multipleSelectFields.length; i++) {
		var fld = $('fld_'+i);
		if(fld){
			multipleSelectFields[i].setFieldDescription(fld.value);
		}
	}
}
function validateForm(){
	if(document.mastersForm.multipleSelectTitle.value.trim()==''){
		alert('<bean:message key="master_add_multiple_select.error.enter_multiple_select_name"/>');
		document.mastersForm.multipleSelectTitle.focus();
		return false;
	}
	if( multipleSelectFields.length==0){
		alert('<bean:message key="master_add_multiple_select.error.enter_options"/>');
		return false;
	}
	for (i = 0; i < multipleSelectFields.length; i++) {
		var fld = $('fld_'+i);
		if(fld){
		
		if(fld.value.trim()==''){
			alert('<bean:message key="master_add_multiple_select.error.enter_option_desc"/>');
			fld.focus();
			return false;
		}
		}
	}
	return true;
}
function constructAndSetFields(){
	updateFieldsArray();
	var str='';
	for (i = 0; i < multipleSelectFields.length; i++) {
		var rf = multipleSelectFields[i];
		if(i>0){
			str +='||';
		}
		var desc = rf.getFieldDescription().replace(/[|]/g ,"&#124;");
		str += rf.getFieldId()+'|'+ desc;
	}
	document.mastersForm.multipleSelectFieldsString.value=str;
}
function saveMultipleSelect(){
	if(validateForm()){
		constructAndSetFields();
		document.mastersForm.submit();
	}
}
function cancelOperation() {
	window.top.hidePopWin(false);
	return false;
}
function onPopUpLoad() {
	window.top.setPopTitle('<b><bean:message key="master_add_multiple_select.title.multiple_select" /></b>');
	populateFields();
}
window.onload = onPopUpLoad;
</script>