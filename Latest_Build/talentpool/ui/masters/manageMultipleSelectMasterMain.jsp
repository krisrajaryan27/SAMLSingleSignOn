...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"selectTitle");
			break;	
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td class="content Grey" style="padding-left:10px; padding-right:10px;"><bean:message key="master_multiple_selects.title.select"/></td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td> 
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="common.name"/>"); 	   	
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=getMultipleSelects");
}

function addNewRecord(){
	var url="masters.do?mode=addMultipleSelect";
	showInPopUp(url,600,500,loadGrid,true);
}

function editRecord(id){
    var url="masters.do?mode=addMultipleSelect&multipleSelectId=" +id;
	showInPopUp(url,600,500,loadGrid,true);
}

function deleteRecord(id){
    var val = dataGrid.getUserData(id,"selectTitle");
    retVal = confirm('<bean:message key="master_multiple_selects.label.confirm_delete_select"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=deleteMultipleSelect&multipleSelectId="+id;
...
function onDeleteComplete(request){
	...
	if(isErrorXml(xmlFile)){
		alert('<bean:message key="master_multiple_selects.error.can_not_delete_select"/>');
		return;
	}
