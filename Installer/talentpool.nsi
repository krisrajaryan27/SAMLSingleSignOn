...
Function revokeSuperUser
	;SetOutPath "$mySQLInstallLocation\bin"
	;StrCpy $scriptFile1 "$INSTDIR\${MYSQL_REMOVE_SUPER_PRIVILEGE}"
...
Function upgradeApp
	...
	File "${BACKUP_UTIL}" 
	File "${RESTORE_UTIL}" 
	File "${RESTORE_DB_UTIL}" 
	File "${MERGE_ATT_TO_DOCS_UTIL}" 
	StrCpy $dbUser ${DB_USERNAME}
	StrCpy $dbPassword ${DB_PASSWORD}	
	
		MessageBox MB_YESNO|MB_ICONQUESTION "Is installed Version of Talentpool older than 15.2?" IDYES uninstallOldSql
		StrCpy $mySQLDecision "1"
		Call takeBackUp
		
	Goto upgradeApplication
...
Function decryptProperties	
	...
	StrCpy $dbPasswordForBackup $dbPassword
	StrCpy $dbUserForBackup $dbuser
...
Function restoreDB
	...
	Push "$INSTDIR\${RESTORE_DB_UTIL}"
	Call AdvReplaceInFile
	nsExec::ExecToLog "$INSTDIR\${RESTORE_DB_UTIL}"
FunctionEnd
...
Function installMySQL
	...
	${Else}
	File /oname=$TEMP\mysql_setup.exe ${MYSQL_SETUP1}
	${EndIf}
...
Function installMySQLForUpgrade
	;File ".\Java Service Wrapper\lib\*.*"
	CreateDirectory "$INSTDIR\mysql"
	SetOutPath "$INSTDIR"
	File "${MYSQL_CREATE_USER_SQL}" 
...
Function uninstallTomact
	; remove all the files and folders for tomact 5.5
...
Function uninstallMySQL
	File /oname=$TEMP\mysql_setup.exe ${MYSQL_SETUP2}
