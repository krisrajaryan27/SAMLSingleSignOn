...
public class OverallRecruitmentCostSummaryChartScriptlet extends JRDefaultScriptlet {
	...
	public void afterReportInit() throws JRScriptletException {
		...
		if (names != null) {
			for (int i = 0; i < names.size(); i++) {
				dataset.setValue((String) names.get(i), (Double) values.get(i));
			}
		}
		JFreeChart chart = ChartFactory.createPieChart3D("", dataset, false, true, false);
		
		PiePlot3D plot = (PiePlot3D) chart.getPlot();
		plot.setLabelGenerator(new OverallRecruitmentCostSummaryChartLabelGenerator());
