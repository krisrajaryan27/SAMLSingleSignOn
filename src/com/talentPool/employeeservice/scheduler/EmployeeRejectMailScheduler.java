 ...
 *
 */
public class EmployeeRejectMailScheduler {
	
	private static Scheduler scheduler;

	private static JobDetail employeeRejectMailJob;

	static {
		try {
			initEmployeeRejectMailJob();
		} catch (Exception e) {
			TPLogger.getLogger().error("Unable to init job for reject email", e);
		}
	}
	
	public static void initEmployeeRejectMailJob() throws SchedulerException {
		if (scheduler == null) {
		...
		if (!jobList.contains(SchedulerConstants.JOB_EMPLOYEE_REJECT_MAIL)) {
			// remmainderjob is volatility=false, durability=true, boolean
			// recover=true
			employeeRejectMailJob = new JobDetail(SchedulerConstants.JOB_EMPLOYEE_REJECT_MAIL, Scheduler.DEFAULT_GROUP, EmployeeRejectMailJob.class, false, true, true);
			scheduler.addJob(employeeRejectMailJob, false);
		}
	}
	...
	private static void addTrigger(String sourceEmail, String userId, String applicantId, String positionId, String positionStepIdFrom) {
		...
		try {
			...
			triggerName = SchedulerConstants.TRIGGER_EMPLOYEE_REJECT_MAIL_SUFFIX + applicantId;
			simpleTrigger = new SimpleTrigger(triggerName, Scheduler.DEFAULT_GROUP, SchedulerConstants.JOB_EMPLOYEE_REJECT_MAIL, Scheduler.DEFAULT_GROUP, new Date(ts), null, 0, 0);
	...
	private static void deleteTrigger(String applicantId) {
		try {
			...
			String triggerName = SchedulerConstants.TRIGGER_EMPLOYEE_REJECT_MAIL_SUFFIX + applicantId;
