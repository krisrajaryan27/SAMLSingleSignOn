...
public class JCryptionUtil {
	 ...
	 */
	public static String decrypt(String encrypted, KeyPair keys) throws RSADecryptionException {
		Cipher dec;
		try {
			dec = Cipher.getInstance("RSA/NONE/NoPadding");
			dec.init(Cipher.DECRYPT_MODE, keys.getPrivate());
		} catch (InvalidKeyException | NoSuchAlgorithmException | NoSuchPaddingException | NullPointerException e) {
			throw new RSADecryptionException("Decrypt error");
		}
		...
		/**
		 * TODO : Some code is getting added in first 2 digits with Jcryption need to investigate
		 */
