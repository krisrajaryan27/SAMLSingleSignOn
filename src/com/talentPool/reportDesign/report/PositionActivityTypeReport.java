 ...
 *
 */
public class PositionActivityTypeReport extends FilterCriteria implements ReportTypes {
	...
	public String getQuery(FilterData filterData, ReportData reportData, ArrayList<ColumnData> columns, String userId, PermissionSet permissionSet){
		...
		query += " JOIN tp_positions tpo ON (tasp.position_id=tpo.position_id) ";
		query += " JOIN tp_applicants tap ON (tasp.applicant_id=tap.applicant_id AND tap.applicant_position_id=tpo.position_id) ";
		
		query += " JOIN tp_user_activity tact ON (tact.position_id=tasp.position_id AND tact.applicant_id=tasp.applicant_id) ";
