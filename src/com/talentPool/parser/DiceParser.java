...
public class DiceParser implements Parser{
	...
	public static final String part7 = "(?mids)(Experience:.*)";
	...
	public static final String part15 = "dice";
	...
	public static final String part29 = "(?mids)(Current Location:.*)";
	public static final String part62 = "(?mids)(Preferred Location:.*)";
	...
	public static final String part35 = "(?mids)(Sent LettersLogout.*)";
	public static final String part36 = "(?mids)(Sent LettersLogout.*)";
			// "(?m)(US Citizen.*)";
	...
	public static final String part61 = "$";
	...
	private static final String regexMobileDice = "[\\s](\\({0,1}\\d{3}\\){0,1}" +
				"[- \\.]\\d{3}[- \\.]\\d{4})|" +
				"(\\+\\d{2}-\\d{2,4}-\\d{3,4}-\\d{3,4})";
	
	public DiceParser(String sourceContent, String textContent) {
		this.sourceContent = sourceContent;
		this.textContent = textContent;
		ParserUtils.loadTechFetchProperties();
	}
	
	public DiceParser(String textContent) {
	...
	public static void main(String args[]){
		...
		String text=cov.convert("D:/projects/v_15_0_0_GA/documents/201505/14/ATT5495511355451578216.html");
		DiceParser dicep=new DiceParser(text);
		dicep.parseApplicantName();
	}
	...
	public void parseCurrentCTC(){
		...
		if (m != null && m.find()) {				
			...
			if(!Utils.isBlankOrNull(value.trim())) {
				...
				if(value.contains(part61)){
					value=value.replace(part61,"");
				}
				currentCTC.add(value);
		...
		try {			
			if (currentCTC.size() > 0 && !Utils.isBlankOrNull(currentCTC.get(0)) && !(currentCTC.get(0)).contains(NotDisclosed)) {
				...
				if(ctc.contains("part61")){
					ctc=ctc.substring(ctc.indexOf("part61")+1, ctc.length());
				}
	...
	public void parseWorkinSince(String exp) {		
		ArrayList<String> totalExp = new ArrayList<String>();//getSingleParseValue(exp);	
		Matcher m = RegexUtils.getMatcher(textContent, part7);		
		// mm/yyyy or mmm-yyyy
		try {
			if(totalExp.size() == 0) {
				String value ="";
	...
	private ArrayList<String> parseMobile() {
		...
		Matcher matcherPhone = RegexUtils.getMatcher(textContent, "[\\s](\\({0,1}\\d{3}\\){0,1}" +
				"[- \\.]\\d{3}[- \\.]\\d{4})|" +
				"(\\+\\d{2}-\\d{2,4}-\\d{3,4}-\\d{3,4})");		
	...
	public void parse() {
		...
		try{
			//name email phone current ctc education
	...
	private void parseApplicantNewCurrentLocation() {
		...
		try {
			if(Utils.isBlankOrNull(currentLocation)){
				Matcher m = RegexUtils.getMatcher(textContent, part62);
				if (m != null && m.find()) {	
