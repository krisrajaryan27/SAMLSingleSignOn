...
import com.talentPool.employeeservice.scheduler.EmployeeDuplicateUploadAttemptJob;

/**
 * @author Shantanu
 *
 */
public class EmployeeDuplicateUploadAttemptJobScheduler {
	private static Scheduler scheduler;

	private static JobDetail employeeDuplicateAttemptJob;

	static {
		try {
			initEmployeeDuplicateAttemptJob();
		} catch (Exception e) {
			TPLogger.getLogger().error("Unable to init job for duplicate attempt", e);
		}
	}

	public static void initEmployeeDuplicateAttemptJob() throws SchedulerException {
		if (scheduler == null) {
		...
		if (!jobList.contains(SchedulerConstants.JOB_EMPLOYEE_DUPLICATE_ATTEMPT)) {
			// remmainderjob is volatility=false, durability=true, boolean
			// recover=true
			employeeDuplicateAttemptJob = new JobDetail(SchedulerConstants.JOB_EMPLOYEE_DUPLICATE_ATTEMPT, Scheduler.DEFAULT_GROUP, EmployeeDuplicateUploadAttemptJob.class, false, true, true);
			scheduler.addJob(employeeDuplicateAttemptJob, false);
		}
	}
	...
	private static void addTrigger(String sourceName, String userId, String userFirstName, String positionId, String positionName, HashMap<String, String[]> nameValues) {
		...
		try {
			...
			triggerName = SchedulerConstants.TRIGGER_EMPLOYEE_DUPLICATE_ATTEMPT_SUFFIX + userId;
			simpleTrigger = new SimpleTrigger(triggerName, Scheduler.DEFAULT_GROUP, SchedulerConstants.JOB_EMPLOYEE_DUPLICATE_ATTEMPT, Scheduler.DEFAULT_GROUP, new Date(ts), null, 0, 0);
	...
	private static void deleteTrigger(String sourceName, String userId, String userFirstName, String positionId, String positionName, HashMap<String, String[]> nameValues) {
		try {
			...
			String triggerName = SchedulerConstants.TRIGGER_EMPLOYEE_DUPLICATE_ATTEMPT_SUFFIX + userId;
