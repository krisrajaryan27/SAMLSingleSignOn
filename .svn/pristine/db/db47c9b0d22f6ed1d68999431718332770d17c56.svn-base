 ...
 *
 */
public class SourceExportManager extends ExportManager {
	public List<SimpleDataObject> getData(String ids, PermissionSet permissionSet) throws SQLException {
		...
		try {
			...
			dynParams[0] = "WHERE tps.source_type_id IN (" + qMarks + ")";
			dq = new DBPreparedQuery("dSourceExportManager_GetData", dynParams);
			int cnt=1;
