...
IN _Type_PreviousEmployer VARCHAR(50),IN _Type_Designation VARCHAR(50),
...
applicant_home_phone ,applicant_cell_phone ,applicant_work_phone ,applicant_working_since,designation,
...
,source_title, source_type_id, reject_reason_ids, reject_step_levels, skill,previous_employer,
...
,' ',case when institute_alias is null then '' else institute_alias end
,' ',case when previous_employer_alias is null then '' else previous_employer_alias end
,' ',case when designation_alias is null then '' else designation_alias end) as alias,
...
group_concat(distinct tempal.alias separator ' ') as previous_employer_alias,
group_concat(distinct templyr.employer_name separator ' ') as previous_employer,
group_concat(distinct tdsgal.alias separator ' ') as designation_alias,
group_concat(distinct tdsgn.designation_name separator ' ') as designation,
...
SET @qry =CONCAT(@qry,", tp_applicant_educational_info taei ");
END IF;
IF STRCMP(_eventType, _Type_PreviousEmployer)=0 THEN
SET @qry =CONCAT(@qry,", tp_applicant_employment_history taeh ");
END IF;
IF STRCMP(_eventType, _Type_Designation)=0 THEN
SET @qry =CONCAT(@qry,", tp_applicant_employment_history taeh ");
END IF;
...
ELSEIF STRCMP(_eventType, _Type_PreviousEmployer)=0 THEN
SET @qry =CONCAT(@qry," AND ta.applicant_id=taeh.applicant_id ");
SET @qry =CONCAT(@qry," AND taei.employer_id =",_eventId);
ELSEIF STRCMP(_eventType, _Type_Designation)=0 THEN
SET @qry =CONCAT(@qry," AND ta.applicant_id=taeh.applicant_id ");
SET @qry =CONCAT(@qry," AND taei.designation_id =",_eventId);
...
LEFT JOIN tp_employer_aliases tempal ON (taeh.employer_id = tempal.employer_id)
LEFT JOIN tp_employers templyr ON (taeh.employer_id = templyr.employer_id)
LEFT JOIN tp_designations_aliases tdsgal ON (taeh.designation_id = tdsgal.designation_id)
LEFT JOIN tp_designations tdsgn ON (taeh.designation_id = tdsgn.designation_id)
