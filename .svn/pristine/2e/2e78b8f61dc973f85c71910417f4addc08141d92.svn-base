...
<html:hidden property="mode" value="duplicateDetectionSettings"/>
...
<div class="contentDivPop" style="padding-right:20px;">
	...
	<table class="boxHeader" cellspacing="0" cellpading="0">
		<tr>
			...
			<td height="18" style="width:265px;"><strong><bean:message key="duplicate_detection_settings.label.vendor"/></strong></td>
			<% } %>
			<% if(ModuleSet.isMODULE_EMPLOYEE()){ %>
			<td height="18" style="width:265px;"><strong><bean:message key="duplicate_detection_settings.label.employee"/></strong></td>
			<% } %>
		</tr>
	</table>
	<table border="0" cellspacing="0" cellpadding="0" class="posinput" style="border:1px solid #99CC33; border-top: none;">
		...
		<tr>	
			...
			<%}	%>
			<% 
				if(idata.getFieldId().equals(DuplicateSettingsConstants.NAME_ID) && idata.getFieldType().equals(DuplicateSettingsConstants.FIELD_TYPE_SYSTEM)){
			%>
			<script type="text/javascript">
			...
			<% if(ModuleSet.isMODULE_VENDOR()){ %>
			<td class="<%=cls %>" style="width:265px;padding-left:4px;padding-right:4px;">
			<% 
				DuplicateSettingsData vdata =null;
				for(int k=0; k<vendorSettings.size(); k++){
					vdata = (DuplicateSettingsData)vendorSettings.get(k);
					if(vdata.getFieldId().equals(data.getFieldId()) && vdata.getFieldType().equals(data.getFieldType())){
						break;
					}
					vdata=null;
				}
				if(vdata!=null){
					String id = vdata.getFieldId() + "_" + vdata.getFieldType() + "_" + vdata.getCheckFor();
					if(Utils.isBlankOrNull(vdata.getFieldCheckType())){ 
			%>
					<input type="hidden" name="<%=id%>"  id="<%=id%>" value=""/>
			  		<img src="images/checkboxunchecked.gif" id="<%=id%>_img" name="<%=id%>_img" onclick="javascript: changeCheckboxState(this);" />				  		
				<% }else{ %>
					<input type="hidden" name="<%=id%>" id="<%=id%>" value="<%=vdata.getFieldCheckType()%>"/>
			  		<img src="images/checkboxchecked.gif" id="<%=id%>_img" name="<%=id%>_img" onclick="javascript: changeCheckboxState(this);" />				  		
				<%} %>	
				<% 
					if(vdata.getFieldId().equals(DuplicateSettingsConstants.NAME_ID) && vdata.getFieldType().equals(DuplicateSettingsConstants.FIELD_TYPE_SYSTEM)){
				%>
				<script type="text/javascript">
					cboVendorName = new SelectBox(opts,'<%=vdata.getFieldCheckType()%>','images/btn_dropdown.gif',{namesonly:false, width:'130px', size:2});
					document.write(cboVendorName.getHtml());
					cboVendorName.init();
					<% if(Utils.isBlankOrNull(vdata.getFieldCheckType())){ %>
					cboVendorName.hideMe();
					<% }%>
				</script>
				<% } %>	
			<% } else{%>
			...
			<td class="<%=cls %>" style="width:265px;padding-left:4px;padding-right:4px;">
			<% 
				DuplicateSettingsData empData =null;
				for(int k=0; k<employeeSettings.size(); k++){
					empData = (DuplicateSettingsData)employeeSettings.get(k);
					if(empData.getFieldId().equals(data.getFieldId()) && empData.getFieldType().equals(data.getFieldType())){
						break;
					}
					empData=null;
				}
				if(empData!=null){
					String id = empData.getFieldId() + "_" + empData.getFieldType() + "_" + empData.getCheckFor();
					if(Utils.isBlankOrNull(empData.getFieldCheckType())){ 
			%>
					<input type="hidden" name="<%=id%>" id="<%=id%>" value=""/>
			  		<img src="images/checkboxunchecked.gif" id="<%=id%>_img" name="<%=id%>_img" onclick="javascript: changeCheckboxState(this);" />				  		
				<% }else{ %>
					<input type="hidden" name="<%=id%>" id="<%=id%>" value="<%=empData.getFieldCheckType()%>"/>
			  		<img src="images/checkboxchecked.gif" id="<%=id%>_img" name="<%=id%>_img" onclick="javascript: changeCheckboxState(this);" />				  		
				<%} %>	
				<% 
					if(empData.getFieldId().equals(DuplicateSettingsConstants.NAME_ID) && empData.getFieldType().equals(DuplicateSettingsConstants.FIELD_TYPE_SYSTEM)){
				%>
				<script type="text/javascript">
					cboEmployeeName = new SelectBox(opts,'<%=empData.getFieldCheckType()%>','images/btn_dropdown.gif',{namesonly:false, width:'130px', size:2});
					document.write(cboEmployeeName.getHtml());
					cboEmployeeName.init();
					<% if(Utils.isBlankOrNull(empData.getFieldCheckType())){ %>
					cboEmployeeName.hideMe();
					<% }%>
				</script>
				<% } %>	
			<% } else{%>
...
function changeCheckboxState(chkBox){
	...
	if(fields[0]=='<%=DuplicateSettingsConstants.NAME_ID%>' && 	fields[1]=='<%=DuplicateSettingsConstants.FIELD_TYPE_SYSTEM%>'){
		...
		}else if(fields[2]=='<%=DuplicateSettingsConstants.SETTINGS_FOR_VENDOR%>'){		
			if(checked){
				cboVendorName.showMe();
			}else{
				cboVendorName.hideMe();
			}
		}else if(fields[2]=='<%=DuplicateSettingsConstants.SETTINGS_FOR_EMPLOYEE%>'){		
			if(checked){
				cboEmployeeName.showMe();
			}else{
				cboEmployeeName.hideMe();
			}
		}
...
function submitfrm(){
	var inernalNameId= '<%=DuplicateSettingsConstants.NAME_ID%>_<%=DuplicateSettingsConstants.FIELD_TYPE_SYSTEM%>_<%=DuplicateSettingsConstants.SETTINGS_INTERNAL%>' ;
	if($(inernalNameId).value!=''){
		$(inernalNameId).value = cboInternalName.getSelectedId();
	}
	<% if(ModuleSet.isMODULE_VENDOR()){ %>
	var vendorNameId= '<%=DuplicateSettingsConstants.NAME_ID%>_<%=DuplicateSettingsConstants.FIELD_TYPE_SYSTEM%>_<%=DuplicateSettingsConstants.SETTINGS_FOR_VENDOR%>' ;
	if($(vendorNameId).value!=''){
		$(vendorNameId).value = cboVendorName.getSelectedId();
	}
	<% } %>
	
	<% if(ModuleSet.isMODULE_EMPLOYEE()){ %>
	var employeeNameId= '<%=DuplicateSettingsConstants.NAME_ID%>_<%=DuplicateSettingsConstants.FIELD_TYPE_SYSTEM%>_<%=DuplicateSettingsConstants.SETTINGS_FOR_EMPLOYEE%>' ;
	if($(employeeNameId).value!=''){
		$(employeeNameId).value = cboEmployeeName.getSelectedId();
	}
	<% } %>
