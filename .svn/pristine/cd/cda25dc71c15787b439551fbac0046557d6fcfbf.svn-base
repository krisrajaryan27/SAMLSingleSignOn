 ...
 * 
 */
public class VendorDuplicateUploadAttemptJob implements Job {
	public void execute(JobExecutionContext context) throws JobExecutionException {
		try {
			TPLogger.getLogger().debug("Start vendor duplicate attempt job");
			...
			if (sendDuplicateNotification.equals(GlobalConstants.ENABLED) && !Utils.isBlankOrNull(userIds)) {
				...
				TemplateData templateData = templateManager.getTemplateDataByTemplateTypeId(TemplateConstants.TEMPLATE_TYPE_DUPLICATE_UPLOAD_TRIED_BY_VENDOR_NOTIFICATION);
			...
			TPLogger.getLogger().debug("End vendor duplicate attempt job");
		} catch (Exception e) {
