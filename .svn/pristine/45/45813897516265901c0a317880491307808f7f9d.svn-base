...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"skillCategory");
			break;	
		case 2:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"skills");
			break;
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;"><bean:message key="admin_master_skills_cat.label.skill_categories_master"/></td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td> 
...
function exportToExcel(){
	var url = "export.do?mode=exportMasters&exportEntityType=<%= ExportConstants.ENTITY_SKILLS%>&ids="+dataGrid.getAllItemIds();
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="admin_master_skill_cat.label.skill_category"/>,<bean:message key="admin_master_skill_cat.label.skills"/>"); 
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageSkillCategories&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function onRowDoubleClick(id){
    if(!id){
     	alert("<bean:message key="admin_master_skill_cat.error.select_to_manage"/>");
     	return;
    }
	manageSkills(id);
}
...
function addNewRecord(){
	var url="masters.do?mode=manageSkillCategoriesMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,250,loadGrid,true);
}
function editRecord(id){
    var url="masters.do?mode=manageSkillCategoriesMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&skillCategory=&skillCategoryId="+id;
	showInPopUp(url,500,250,loadGrid,true);
}


function deleteRecord(id){
    var val = dataGrid.getUserData(id,"skillCategory");
    retVal = confirm('<bean:message key="admin_master_skill_cat.label.confirm_delete"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=manageSkillCategories&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&skillCategoryId="+id;
...
function onDeleteComplete(request){
	...
	if(isErrorXml(xmlFile)){
		alert('<bean:message key="admin_master_skill_cat.error.failed_delete"/>');
		return;
	}
...
function manageSkills(id){
	//go manage skill for skill category
	window.location.href="masters.do?mode=manageSkillsMaster&skillCategoryId="+id+"&t=<bean:write name="mastersForm" property="t"/>&st=<bean:write name="mastersForm" property="st"/>";
}
...
function importFromExcel(){
	var url = "import.do?mode=importMasters&importEntityType=<%= ExportConstants.ENTITY_SKILLS%>";
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}
...
function startExcelImport(filePath){	
	...
	document.importForm.entityType.value='<%= ExportConstants.ENTITY_SKILLS%>';
