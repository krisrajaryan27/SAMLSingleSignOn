...
<div class="outerDiv"	style="width:450px;padding:10px;height:80px; overflow: auto; padding: 20px;">
	<table cellspacing="0" cellpading="0" width="100%">
		<tr>
			<logic:equal value="<%=ApplicantConstants.APPLICANT_STATUS_BLACKLISTED%>" name="applicantForm" property="duplicateApplicantStatus">
			<td style="color: red;">
				<bean:message key="common.applicant"/>&nbsp;<bean:message key="black_list.import.duplicate" />
			</td>
			</logic:equal>
			<logic:equal value="<%=ApplicantConstants.APPLICANT_STATUS_NORMAL%>" name="applicantForm" property="duplicateApplicantStatus">
			<td >
				<bean:message key="duplicate_applicant.label.replace_duplicate_resume_with_new" />
			</td>
			</logic:equal>
		</tr>
	</table>
	<div class="navBtn" style="float: left;margin-top:20px; ">
		<logic:equal value="<%=ApplicantConstants.APPLICANT_STATUS_NORMAL%>" name="applicantForm" property="duplicateApplicantStatus">
			<a href="#" style="width:60px; margin-right: 5px;" class="active" onclick="javascript:yes();return false;" id="ignore">
			<span class="rightC"></span><span class="leftC"></span><bean:message key="common.yes" /></a>
			<a href="#" style="width:60px; margin-right: 5px;" class="active" onclick="javascript:no();return false;" id="ignore">
			<span class="rightC"></span><span class="leftC"></span><bean:message key="common.no" /></a>
		</logic:equal>
		<logic:equal value="<%=ApplicantConstants.APPLICANT_STATUS_BLACKLISTED%>" name="applicantForm" property="duplicateApplicantStatus">
			<a href="#" style="width:60px; margin-right: 5px;" class="active" onclick="javascript:no();return false;" id="ignore">
...
function yes(){
	...
	document.applicantForm.applicantId.value= document.applicantForm.duplicateApplicantId.value;
