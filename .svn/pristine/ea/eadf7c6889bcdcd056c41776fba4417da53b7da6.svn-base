...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;">Flags Master</td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td>  
    <td>
		...
		<a href="#" style="width:80px;" onclick="editRecord('');"><span class="rightC"></span><span class="leftC"></span><bean:message key="common.add_new"/></a> 
		</div>			    
    </td>    
  </tr> 
</table> 
...
function initGrid(){
        dataGrid = new dhtmlXGridObject('dataGrid'); 
	   	...
	   	dataGrid.setHeader("&nbsp;,&nbsp;,<bean:message key="flag_master.label.description"/>,<bean:message key="flag_master.label.type"/>,<bean:message key="flag_master.label.owner"/>"); 
	   	dataGrid.setInitWidths("18,18,200,200,280");
	   	dataGrid.setColAlign("left,left,left,left,left");
	   	<logic:equal value="true" name="permissionSet" scope="session" property="PERMISSION_MANAGE_ALL_FLAGS">
	   	dataGrid.setColTypes("ro,ro,link,ro,ro"); 
	   	</logic:equal>
	   	<logic:notEqual value="true" name="permissionSet" scope="session" property="PERMISSION_MANAGE_ALL_FLAGS">
	   	dataGrid.setColTypes("ro,ro,ro,ro,ro"); 
	   	</logic:notEqual>
	   	dataGrid.setColSorting("na,na,str,na,na");
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageFlagMaster&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function editRecord(id){
    var url="masters.do?mode=manageFlag&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&flagId="+id;
...
function deleteRecord(id){
    var val = dataGrid.getUserData(id,"Col_II_Comment");
    retVal = confirm('<bean:message key="flag_master.label.confirm_delete_flag"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=deleteFlag&flagId="+id;
...
function onDeleteComplete(request){
	...
	if(isErrorXml(xmlFile)){
		alert('<bean:message key="flag_master.error.can_not_delete_flag"/>');
		return;
	}
