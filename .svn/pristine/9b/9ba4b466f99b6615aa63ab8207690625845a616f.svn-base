...
<div class="contentDivPop" style="width:450px;">
	...
	<html:form action="/masters" onsubmit="submitForm();return false;">
  	<html:hidden property="instituteId" name="mastersForm"/>
	...
	<div class="popupTop">
		<table class="tblPop">
		   <tr>
			  <td class="header">
				  <bean:message key="admin_master_institute_cat.label"/>
				  <span class="star">*</span>
				  :
			  </td>
			  <td>
				  <html:text property="instituteName" name="mastersForm" size="35" styleId="skillCategory"></html:text>
			  </td>
		  </tr>
		  <tr>
			  <td class="header">
...
function submitForm(){
	var Name = document.mastersForm.instituteName.value;
	if(Name.trim()==""){
		return;
	}
	//alert('submit');	
...
function actionOnLoad(){
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_ADD%>">
		window.top.setPopTitle('<b><bean:message key="admin_master_institute.label.add_institute"/></b>');
	</logic:equal>
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_EDIT%>">
		window.top.setPopTitle('<b><bean:message key="admin_master_institute.label.edit_institute"/></b>');
	</logic:equal>
}
