...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"locationName");
			break;	
		case 2:
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td class="content Grey" style="padding-left:10px; padding-right:10px;"><bean:message key="master_locations.title.locations"/></td>
			<td class="rightC"></td>
		</tr>
...
function exportToExcel(){
	var url = "export.do?mode=exportMasters&exportEntityType=<%= ExportConstants.ENTITY_LOCATIONS%>&ids="+dataGrid.getAllItemIds();
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="master_locations.label.location"/>"); 	   	
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("location.do?mode=getLocations");
}

function addNewRecord(){
	var url="location.do?mode=addLocation";
	showInPopUp(url,500,250,loadGrid,true);
}

function editRecord(id){
    var url="location.do?mode=addLocation&locationId=" +id;
	showInPopUp(url,500,250,loadGrid,true);
}

function deleteRecord(id){
    var val = dataGrid.getUserData(id,"locationName");
    retVal = confirm('<bean:message key="master_locations.label.confirm_delete_location"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=deleteLocation&locationId="+id;
...
function manageOffices(id){
	window.location.href="location.do?mode=manageOffices&locationId="+id;
}
...
function importFromExcel(){
	var url = "import.do?mode=importMasters&importEntityType=<%= ExportConstants.ENTITY_LOCATIONS%>";
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}
...
function startExcelImport(filePath){	
	...
	document.importForm.entityType.value='<%= ExportConstants.ENTITY_LOCATIONS%>';
