 ...
 *
 */
public class PositionCandidateTypeReport extends FilterCriteria implements ReportTypes {
	...
	public String getFilter() {
		...
		try {
				...
				sb.append("new SelectOption('" + ReportDesignConstants.FILTER_APPOINTMENT + "','" + "Appointment Filter" + "'),");
				sb.append("new SelectOption('" + ReportDesignConstants.FILTER_USER + "','" + "User Filter" + "')");
				sb.append("]");
	...
	public String getQuery(FilterData filterData, ReportData reportData, ArrayList<ColumnData> columns, String userId, PermissionSet permissionSet){
		...
		query += " ( ";
		query += " SELECT * FROM ( ";
		query += " 	SELECT * FROM tp_applicant_selection_process ORDER BY process_id DESC ) AS tasp ";
		query += " 	GROUP BY tasp.applicant_id, tasp.position_id ";
		query += " 	)AS tasp ";
		query += " LEFT JOIN tp_positions tpo ON (tasp.position_id=tpo.position_id) ";
		query += " LEFT JOIN tp_applicants tap ON (tasp.applicant_id=tap.applicant_id) ";
		...
		// Appointment
		query += " LEFT JOIN tp_appointments tpp ON (tpp.applicant_id=tap.applicant_id " 
				+" AND tpp.position_id=tap.applicant_position_id " 
				+" AND tpp.position_step_id=tap.applicant_step_id "
				+" AND tpp.appointment_status_id!= " + CalendarConstants.APPOINTMENT_STATUS_NOSHOW
				+" AND tpp.appointment_status_id!= " + CalendarConstants.APPOINTMENT_STATUS_HAPPENED
				+" ) ";
		query += " LEFT JOIN tp_appointment_attendees tppa ON (tpp.appointment_id=tppa.appointment_id) ";
		query += " LEFT JOIN tp_appointment_status tapps ON (tapps.appointment_status_id=tpp.appointment_status_id) ";
		query += " LEFT JOIN tp_users tuaa ON(tuaa.user_id=tppa.attendee_id) ";
		query += " LEFT JOIN tp_users tuac ON(tuac.user_id=tpp.appointment_created_by) ";
		query += " LEFT JOIN tp_users tuam ON(tuam.user_id=tpp.appointment_modified_by) ";
		...
		query += " LEFT JOIN tp_position_step_users tpsu ON (tasp.position_id=tpsu.position_id AND tpsu.position_step_id=tps.position_step_id) ";
		query += " LEFT JOIN tp_users tusat ON(tusat.user_id=tpsu.user_id AND tpsu.is_responsible_for_scheduling=0 AND tpsu.is_authorized_to_move=0) ";
		query += " LEFT JOIN tp_users tussb ON(tussb.user_id=tpsu.user_id AND tpsu.is_responsible_for_scheduling=1 AND tpsu.is_authorized_to_move=0) ";
		query += " LEFT JOIN tp_users tusdm ON(tusdm.user_id=tpsu.user_id AND tpsu.is_responsible_for_scheduling=0 AND tpsu.is_authorized_to_move=1) ";
		...
		/* Section 3*/	
		query += " WHERE 1 ";
		query= addFilterCriteria(query, filterData, reportData, "tap.applicant_date_created", "tpsu.user_id");
