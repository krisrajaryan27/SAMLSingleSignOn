 ...
 *
 */
public class BranchExportManager extends ExportManager {
	public List<SimpleDataObject> getData(String ids, PermissionSet permissionSet) throws SQLException {
		...
		try {
			...
			dynParams[0] = "WHERE tpb.branch_id IN (" + qMarks + ")";
			dq = new DBPreparedQuery("dBranchExportManager_GetData", dynParams);
			int cnt=1;
