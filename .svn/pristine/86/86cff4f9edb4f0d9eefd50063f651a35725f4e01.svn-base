...
<div class="contentDivPop" style="width:450px;">
	...
	<html:form action="/masters" onsubmit="submitForm();return false;">
  	<html:hidden property="costCenterId" name="mastersForm"/>
	...
	<div class="popupTop">
		<table class="tblPop">
		   <tr>
			  <td class="header">
				  <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_COST_CENTER_LABEL)%>&nbsp;<bean:message key="master_cost_center.label.name"/>
				  <span class="star">*</span>
				  :
			  </td>
			  <td>
				  <html:text property="costCenterName" name="mastersForm" size="35" styleId="costCenterName"></html:text>
			  </td>
		  </tr>
		  <tr>
			  <td class="header">
				 <bean:message key="master_cost_center.label.description"/> 
			  </td>
			  <td>
...
function submitForm(){
	var errors = '';
	var Name = document.mastersForm.costCenterName.value;
	if(Name.trim()==""){
		//return;
...
function actionOnLoad(){
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_ADD%>">
		window.top.setPopTitle('<b><bean:message key="common.add"/> <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_COST_CENTER_LABEL)%></b>');
	</logic:equal>
	<logic:equal name="mastersForm" property="subMode" value="<%=MastersConstants.SUB_MODE_EDIT%>">
		window.top.setPopTitle('<b><bean:message key="common.edit"/> <%=GlobalApplicationProperties.getProperty(GlobalConstants.PROPERTY_COST_CENTER_LABEL)%></b>');
	</logic:equal>
}
