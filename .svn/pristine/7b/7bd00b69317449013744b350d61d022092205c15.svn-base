...
function getCustomTitle(obj){
	...
	switch(obj.cell._cellIndex){
		case 0:
			...
			break;
		case 1:
			return obj.grid.getUserData(obj.cell.parentNode.idd,"sourceType");
			break;	
		case 2:
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"> 
  <tr> 
    <td  valign="bottom">
		...
		<tr>
			<td class="leftC"></td>
			<td id="monthYear" class="content Grey" style="padding-left:10px; padding-right:10px;"><bean:message key="admin_master_source.label.source_categories_master"/></td>
			<td class="rightC"></td>
		</tr>
		</table>
    </td> 
...
function exportToExcel(){
	var url = "export.do?mode=exportMasters&exportEntityType=<%= ExportConstants.ENTITY_SOURCES%>&ids="+dataGrid.getAllItemIds();
...
function initGrid(){
	   	...
	   	dataGrid.setHeader("&nbsp;,<bean:message key="admin_master_source.label.source_category"/>, <bean:message key="admin_master_source.label.source_types"/>"); 	   	
...
function loadGrid(){
	dataGrid.clearAll();
	dataGrid.loadXML("masters.do?mode=manageSourceTypes&subMode=<%=MastersConstants.SUB_MODE_GET%>");
}

function onRowDoubleClick(id){
    if(!id){
     	alert("<bean:message key="admin_master_source.error.select_sourceType_to_manage"/>");
     	return;
    }
	manageSources(id);
}

function manageSources(id) {
    window.location.href="masters.do?mode=manageSourcesMaster&sourceTypeId=" + id ;
}

function addNewRecord(){
	var url="masters.do?mode=manageSourceTypesMaster&subMode=<%=MastersConstants.SUB_MODE_ADD%>";
	showInPopUp(url,500,250,loadGrid,true);
}
function editRecord(id){
    var url="masters.do?mode=manageSourceTypesMaster&subMode=<%=MastersConstants.SUB_MODE_EDIT%>&category=&sourceTypeId="+id;
	showInPopUp(url,500,250,loadGrid,true);
}


function deleteRecord(id){
    var val = dataGrid.getUserData(id,"sourceType");
    retVal = confirm('<bean:message key="admin_master_source.label.confirm_delete_source_type"/>' + ' ' + val + '. ' + '<bean:message key="common.continue?"/>');
    if (retVal == true) {
		var pars = "mode=manageSourceTypes&subMode=<%=MastersConstants.SUB_MODE_DELETE%>&sourceTypeId="+id;
...
function onDeleteComplete(request){
	...
	if(isErrorXml(xmlFile)){
		alert('<bean:message key="admin_master_source.errors.cannot_delete_srcCategory"/>');
		return;
	}
...
function importFromExcel(){
	var url = "import.do?mode=importMasters&importEntityType=<%= ExportConstants.ENTITY_SOURCES%>";
	window.setTimeout("showInPopUp('"+url+"',550,320,null,true);", 10);
}
...
function startExcelImport(filePath){	
	...
	document.importForm.entityType.value='<%= ExportConstants.ENTITY_SOURCES%>';
