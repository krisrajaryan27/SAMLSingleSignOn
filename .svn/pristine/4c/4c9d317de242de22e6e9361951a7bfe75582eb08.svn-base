...
CREATE PROCEDURE `PROC_GET_TODO_LIST`(
	...
	IN _appJoined INT, IN _positionStepLevelApprove INT, IN _positionStepLevelSelect INT,
	IN _includeRequisitionToDo INT,	IN _positionStatusInprocess INT, 
	IN _todoSchedule INT, IN _todoConfirmAttendance INT,
...
DECLARE cur_my_applicants CURSOR FOR 
	...
	AND ta.applicant_joined= _appJoined 
...
REPEAT
	...
	if v_noRecord =0 THEN
		IF v_position_step_isscheduled = 1 THEN
			...
			SELECT tapo.appointment_id, appointment_from_date, attendee_id 
			FROM 
			...
			INTO v_appointment_id, v_appointment_from_date, v_attendee_id;
			...
			ELSE
				IF v_appointment_from_date < now() THEN
					...
					IF v_noRecord =1 OR vv_process_id IS null THEN
						...
						IF v_is_responsible_for_scheduling =1 OR ( v_is_interviewer_can_confirm=1 AND v_is_responsible_for_scheduling=0 AND (SELECT COUNT(*) FROM tp_appointment_attendees taa 
						WHERE taa.appointment_id=v_appointment_id AND taa.attendee_id=_userId)>0) THEN
